...
- `darwinModules.{core, macos-defaults, homebrew}`, aggregated by `default`:
  ...
  - `bw-broker` — the [bw-broker](https://github.com/donq-io/donq_bw-broker) launchd service (approval-gated Bitwarden credentials for coding agents), wired to `pkgs.donq.bw-broker` and defaulting `allowedUsers` to the machine's user. Options only: nothing is created until a machine sets `services.bw-broker.enable = true`, which is why it can sit in `default`.
- `homeManagerModules.{dev-tools, git, shell, dotfiles}`, aggregated by `default`. `dev-tools` expects `pkgs.unstable` and `pkgs.donq.*` (see overlays below).
...
- `overlays.donq-cli-tools` — exposes the in-house inputs (`pd` from donq_productive-cli, `zm` from donq_zammad-cli, `bw-broker` from donq_bw-broker) as `pkgs.donq.*`.
...
- The in-house inputs (`productive-cli`, `zammad-cli`, `bw-broker`) are private GitHub repos, so building requires a GitHub token that can read them — and, since they are plain flake inputs, *every* machine fetches them at lock time whether or not it uses them; a repo the token cannot read breaks `snix` fleet-wide, so grant the bot access before merging a new input. Access is centralized on the `nixbot-DQ` machine user (read-only `pull` grants, managed in donq_infrastructure/github): MDM ships its token as an `access-tokens = github.com=…` line in `/etc/nix/nix.custom.conf` — it must land there before the first `darwin-rebuild` (builds fetch the CLI sources) — and CI reads the same token from the `NIXBOT_DQ_PAT` repo secret (set manually — deliberately not Terraform-provisioned, keeping the token out of the Terraform Cloud state). Hand-rolled consumers use any token of their own (e.g. `gh auth token`). This repo itself is still public; flipping it private is deferred until the token rollout has proven itself — the bot already holds a dormant `pull` grant, so the flip is a one-word change in donq_infrastructure, but it also moves the token requirement earlier in bootstrap (before the templater `nix run`).
- There is deliberately no build/test CI (decision, 2026-08): fleet breakage from the daily lock bumps surfaces only when a machine runs `snix`. Revisit if the fleet grows.
